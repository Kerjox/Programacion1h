...
public class Hampon extends Rectangle implements Runnable{

	public Thread animation;
	private ArrayList<Image> images;
	private int indexImage;

	public Hampon(int x, int y) {
	...
	private void loadImagesGuerrillero() {
		...
		for (int i = 1; i <= 4; i++) {
			...
			try {
				imageToLoad = ImageIO.read(getClass().getResource("./Sprites/Hampon/h" + i + ".gif"));
			} catch (IOException e) {
				e.printStackTrace();
			}
